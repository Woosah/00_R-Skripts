...
# http://www.mikemeredith.net/blog/1309_SECR_in_BUGS_patchy_habitat.htm
...
wantedXY <- c("N", "D", "psi", "lam0", "sigma", "SX", "SY")
...
plot(win, main="50 simulated ACs\nfor each animal captured")
for(i in 1:4) 
  points(x=res[, i+2], y=res[, i+52], pch=19, cex=0.3, col=i)

# Same for 4 animals NOT captured
plot(win, main="50 simulated ACs\nfor 4 animals not captured")
for(i in 5:8) 
  points(x=res[, i+2], y=res[, i+52], pch=19, cex=0.3, col=i-4)
# Some of these points seem to be just outside the habitat patch
# Display the pixel grid as well:
for(x in 1:50)   for(y in 1:46)    if(habmat[x, y])
      points(x-0.5, y-0.5, pch=22, col='grey', bg='grey', cex=1.3)
# Replot the outline and the points
plot(win, add=TRUE, lwd=3)
for(i in 5:8) 
  points(x=res[, i+2], y=res[, i+52], pch=19, cex=0.3, col=i-4)
# OK, it's because the pixels don't match the outline exactly.


# Plot ACs for all animals not captured
allx <- as.vector(res[, 7:52])
ally <- as.vector(res[, 57:102])
plot(win, main="50 ACs for each animal not captured")
for(x in 1:50)   for(y in 1:46)    if(habmat[x, y])
      points(x-0.5, y-0.5, pch=22, col='grey', bg='grey', cex=1.3)
plot(win, lwd=3, add=TRUE)
points(allx, ally, pch=19, cex=0.3, col='blue')
